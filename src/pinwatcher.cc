 ...
 * and back while watching a gpio pin and waiting for it to interrupt.
 */
struct Baton {
    uv_work_t request;

    // The number of the gpio pin to watch.
    uint32_t pin;

    // The JavaScript callback to call when the gpio pin interrupts.
    v8::Persistent<v8::Function> callback;

    // The value of the gpio pin being watched on interrupt (0 or 1.)
    uint32_t value;
...
v8::Handle<v8::Value> Watch(const v8::Arguments& args) {
    ...
    if (!args[0]->IsUint32()) {
        return ThrowException(v8::Exception::TypeError(
            v8::String::New("First argument must be a gpio pin number")
        ));
    }
...
void WatchWork(uv_work_t* req) {
    ...
    char filename[1024];
    sprintf(filename, "/sys/class/gpio/gpio%d/value", baton->pin);
            ...
            if (ready == -1) {
                ...
                if (offset == -1) {
                    ...
                    if (size == -1) {
                        ...
                        baton->error = true;
                        baton->error_message = "No valid pin value found.";
                    }
                }
            } else {
                baton->error = true;
...
NODE_MODULE(pinwatcher, RegisterModule);
